//right
/*
for (int i = c->center.x + 1; i < c->center.x + c->radius; i++) {
    if (c->center.y < 0) break;
    if (i >= width - 1 || i < 0) continue;

    cv[c->center.y][i] = circleOut;
}
*/
...
/*void drawRectangle(int height, int width, char cv[height][width], Rectangle *r) {
    ...
    //cant be split into function because of lots of small differences

    //top line
    for (int i = topLeft.x+1; i < topRight.x; i++) {
        if (topRight.y > height - 1 || topRight.y < 0) {
            break;
        }
        if (i >= width - 1 || i < 0) {
            continue;
        ...
        cv[topRight.y][i] = horizontal;
    }

    //right line
    for (int i = topRight.y-1; i > bottomRight.y; i--) {
        if (topRight.x > width - 2 || topRight.x < 0) {
            break;
        }
        if (i > height - 1 || i < 0) {
            continue;
        ...
        cv[i][topRight.x] = vertical;
    }

    //bottom line
    for (int i = bottomRight.x-1; i > bottomLeft.x; i--) {
        if (bottomLeft.y > height - 1 || bottomLeft.y < 0) {
            break;
        }
        if (i >= width - 1 || i < 0) {
            continue;
        ...
        cv[bottomRight.y][i] = horizontal;
    }

    //left line
    for (int i = bottomLeft.y+1; i < topLeft.y; i++) {
        if (topLeft.x > width - 2 || topLeft.x < 0) {
            break;
        }
        if (i > height - 1 || i < 0) {
            continue;
...
void drawRectangle(int height, int width, char cv[height][width], Rectangle *r) {
    ...
    for (int i = 0; i < w; i++) {
        if (x < width - i && y < height - 1 && x >= -i && y >= 0) {
            cv[y][x + i] = horizontal;
        }
        if (x < width - w + i && y < height - h - 1 && x >= i - w && y >= -h) {
            cv[y + h][x + w - i] = horizontal;
        }
    }
    for (int i = 0; i < h; i++) {
        if (x < width && y < height - i - 1 && x >= 0 && y >= -i) {
            cv[y + i][x] = vertical;
        }
        if (x < width - w && y < height + i - h && x >= - w && y >= i - h) {
...
void drawRectangleCornerCheck(Point *a, int height, int width, char cv[height][width]) {
    if (a->x < width - 1 && a->y < height - 2 && a->y >= 0 && a->x >= 0) {
        cv[a->y][a->x] = corner;
    }
}
...
void drawCircle (int height, int width, char cv[height][width], Circle *c) {
    ...
    for (int x = cX - r; x < cX + r; x++) {
        for (int y = cY - r; y < cY + r; y++) {
            if (x <= height -1 && y <= width - 2 && x >= 0 && y >= 0) {
    ...
    //center
    if (c->center.y <= height -1 && c->center.x <= width - 2 && c->center.x >= 0 && c->center.y >= 0) {
